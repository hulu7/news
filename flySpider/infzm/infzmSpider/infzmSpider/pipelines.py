...
class InfzmspiderToMongodbPipeline(object):
    ...
    def from_crawler(cls, crawler):
        return cls(
            ...
            mongo_db=crawler.settings.get('MONGO_DATABASE', 'infzm'),
    ...
    def process_item(self, item, spider):
        if isinstance(item,InfzmspiderToMongodbItem):
            self._process_infzmspider_item(item)
        return item

    def _process_infzmspider_item(self,item):
        mongodbItem = dict(item)
        mongodbItem.pop('content')
        self.db.contentInfo.insert(mongodbItem)

class InfzmspiderToTxtPipeline(object):
    def process_item(self, item, spider):
