...
mongoexport -d 'huxiu' -c 'contentInfo' --csv -f share_number,comment_number,url,title,author_name,image_url,author_url,time,id -o ${tmpDir}/huxiu_backup_tmp.csv
...
mongoexport -d 'ifeng_content' -c 'contentInfo' --csv -f id,title,time,comment_number,join_number,url,author_name -o ${tmpDir}/ifeng_content_tmp.csv
