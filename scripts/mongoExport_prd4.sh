...
do
    if [ "${camel}" != "__init__.py" ]; then
        ...
        mongoexport -d ${domain_name} -c 'contentInfo' --csv -f id,title,url,author_name,public_time,download_time,is_open_cache,source -q "{'download_time': '${DATE}'}" -o ${tmpDir}/${domain_name}_content_tmp.csv
