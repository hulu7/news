...
do
    ...
    mongoexport -d 'SPIDERS' -c ${domain} --type=csv -f id,new_id,url,title,author_name,public_time,download_time,is_open_cache,source,images -q "{'public_time': '${DATE}'}" -o ${tmpDir}/${domain}_content_tmp.csv
