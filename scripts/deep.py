...
def excute(name):
    ...
    finished_id = []
    saveCSVFilePath = '/home/dev/Repository/news/scripts/deep_ifeng.csv'
    saveTxtFilePath = '/home/dev/Repository/news/scripts/deep_cache.txt'
    csv_file = open(saveCSVFilePath, 'r')
    finished_files = csv.reader(csv_file)
    for file in finished_files:
        finished_id.append(file[0])
    csv_file.close()
    count = len(finished_id)
    csv_file = open(saveCSVFilePath, 'a')
    csv_writer = csv.writer(csv_file)
    if count == 0:
        csv_writer.writerow(['id', 'deep'])
    for file in files:
        id = re.findall(r"\d+\.?\d*", file.decode('utf-8'))
        if str(id[0]) in finished_id:
            continue
        count += 1
        deepMax = deep.howDeep(path + '/' + file)
        print str(id[0]) + ' ---- ' + str(deepMax) + ' ---- ' + str(count) + '\n'
        deep.writeToTxt(saveTxtFilePath, str(count))
        csv_writer.writerow([str(id[0]), str(deepMax)])
    csv_file.close()
